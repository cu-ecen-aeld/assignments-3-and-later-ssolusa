...
qemu-system-aarch64 -m 256M -M virt -cpu cortex-a53 -nographic -smp 1 \
        -kernel ${KERNEL_IMAGE} \
        -initrd ${INITRD_IMAGE} \
        -chardev stdio,id=char0,mux=on,logfile=${OUTDIR}/serial.log,signal=off \
        -serial chardev:char0 -mon chardev=char0 \
        -append "console=ttyAMA0 rdinit=/sbin/sh"
        # ssolusa; added above "console=ttyAMA0 b/c error: Run /bin/sh as init process /bin/sh: can't access tty; job control turned off
        #          but still errors
